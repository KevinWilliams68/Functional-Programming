 ...
 * Your code should replace undefined (). *)
let name : string = first_name ^ "Williams";;

(* 1.c. Create a string containing your email address.
 * Your code should replace undefined (). *)
let email : string = "williamskevin1968@gmail.com";;

(* 1.d. Create an integer with your (estimated) graduate year.
 * Your code should replace undefined (). *)
let class_year : int = 2022;;

(* 1.e. Replace the ??? with what you're excited about in this course. *)
let exciting : string = "I'm excited about learning a new programming language!";;
...
let exp3a : string = "4 is an int and 3.9 is a float. Add a . to the 4 or remove . to make values the same";;
...
let exp3b : string = "() should be added around the operations in call to the aux function";;
...
let exp3c : string = "sumTo is recursive and needs rec in the declaration";;
...
(* Your code should replace undefined (). *)
let exp4b : string = "The function's problem is it doesn's display the output, Input is not terminated";;
...
(*
let rec forever (x:bool) : bool =
  forever forever
;;
*)
let exp4c : string = "It is impossible. The function will always have an output type
and input type which is not included in 'forever forever' ";;
...
let rec few_divisors (n:int) (m:int) : bool =
  ...
  let rec check (n:int) (index:int) : int =
    match index with
    1 -> 1
    | _ -> if (n mod index) = 0
      then check (n) (index - 1) + 1
      else check (n) (index - 1)
    in 
  if check (n) (n) < m then true else false 
;;
...
let rec sin_pi (n:int) : float =
  if n < 0 then bad_arg n
    ...
    let rec approx (n:int) : float =
      match n with
      0 -> 3.
      | _ -> approx(n - 1) +. sin(approx(n - 1))
    in
    approx n
;;
...
let monte_pi (n : int) : float = 
  if n < 0 then bad_arg (n)
    ...
    let square j = j *. j in
    let rec count (index : int) : int = 
      let (x, y) = (Random.float 1., Random.float 1.) in
      match index with 
        0 -> 0
      | _ -> if (square x +. square y <= 1.) 
        then (count (index - 1) + 1) 
        else count (index - 1)
    in 
  float_of_int (count n) /. float_of_int (n) *. 4.
;;
...
(* Your code should replace undefined (). *)
let exp7a : string = "This is called the Madhava-Leibniz series. It finds pi / 4 by adding the infinite
sum of (-1)^(n-1) / (2n - 1), but to find pi, we need to multiply the final output by 4. 
'N' is any integer and the larger the number, the greater the accuracy of pi we recieve. 
Source: https://en.wikipedia.org/wiki/Madhava_of_Sangamagrama , under the section titled
'The value of pi' ";;
...
(* Your code should replace undefined (). *)
let exp7b : string = "The parameter 'n' is used to represent the sum of the
 first 'n' terms in the infinite sum";;

let rec your_pi (n:int) : float =
  if n < 0 then bad_arg(n)
  else
    let rec power (s:float) (t:int) : float =
      match t with
      0 -> 1.
      | _ -> s *. power s (t - 1)
    in
    let rec equation (k : int) : float = 
      match k with
      0 -> 1.
      | _ -> (power (-1.) (k - 1) ) /. ((2. *. float_of_int k) -. 1.) +. equation (k - 1) 
    in
    4. *. (equation n -. 1.)
;;
...
assert (your_pi 1 = 4.);;
assert (your_pi 5 = 3.33968253968254025);;
assert (your_pi 9999 = 3.14169266359057975);;
(* MORE TEST CASES *)
